...
// ... (Keep Interfaces and normalize functions from previous steps) ...
interface AttributeScore { attribute: string; sentiment_score: number; context_quote: string; }
interface Insight { brand_name: string; product_name: string; product_category: string; overall_sentiment: 'positive' | 'negative' | 'neutral' | 'mixed'; specific_variant?: string; active_ingredients?: string[]; attributes?: AttributeScore[]; }
const BRAND_MAPPING: Record<string, string> = { "loreal": "L'Oréal", "l'oreal": "L'Oréal", "l'oréal": "L'Oréal", "l'oreal paris": "L'Oréal", "l'oréal paris": "L'Oréal", "maybelline": "Maybelline", "maybelline new york": "Maybelline", "elf": "e.l.f. Cosmetics", "e.l.f.": "e.l.f. Cosmetics", "elfcosmetics": "e.l.f. Cosmetics", "charlotte tilbury": "Charlotte Tilbury", "fenty": "Fenty Beauty", "fenty beauty": "Fenty Beauty", "rare beauty": "Rare Beauty", "la prairie": "La Prairie", "kylie": "Kylie Cosmetics", "kylie cosmetics": "Kylie Cosmetics", "rem beauty": "r.e.m. beauty", "r.e.m. beauty": "r.e.m. beauty" };
const normalizeBrand = (brand: string): string => { if (!brand) return 'Unknown Brand'; const lookup = brand.trim().toLowerCase(); if (BRAND_MAPPING[lookup]) return BRAND_MAPPING[lookup]; for (const [key, canonical] of Object.entries(BRAND_MAPPING)) { if (lookup.includes(key) || key.includes(lookup)) return canonical; } return brand; };
const normalizeProduct = (product: string): string => { if (!product) return 'Unknown Product'; let cleaned = product.toLowerCase().replace(/\b\w/g, s => s.toUpperCase()); cleaned = cleaned.replace(/Colour/g, 'Color'); return cleaned.trim(); };
...
function AnomalyDetectionPanel({ data, minimumSampleVolume = 3, onNavigate }: { data: any[], minimumSampleVolume?: number, onNavigate: (b: string, p: string, v: string, sentiment: string) => void }) {
  const anomalies = useMemo(() => {
    ...
    data.forEach(insight => {
      ...
      if (!productStats[prodKey]) productStats[prodKey] = { brand: insight.brand_name, product: insight.product_name, globalScores: [], variants: {} };
      if (!productStats[prodKey].variants[variant]) productStats[prodKey].variants[variant] = [];
      const avgScore = insight.attributes?.reduce((sum: number, attr: any) => sum + attr.sentiment_score, 0) / (insight.attributes?.length || 1);
      if (!isNaN(avgScore)) { productStats[prodKey].globalScores.push(avgScore); productStats[prodKey].variants[variant].push(avgScore); }
    });
    ...
    Object.entries(productStats).forEach(([prodKey, stats]) => {
      ...
      Object.entries(stats.variants).forEach(([variant, scores]) => {
        ...
        if (Math.abs(diff) > (stdDev * 1.5)) flagged.push({ brand: stats.brand, product: stats.product, variant, globalMean, variantMean, deviation: diff, sampleSize: scores.length });
      });
    });
...
function CompareDashboardContent() {
  ...
  const normalizedData = useMemo(() => {
    return rawData.map(item => ({ 
      ...item, brand_name: normalizeBrand(item.brand_name), product_name: normalizeProduct(item.product_name)
    }));
  }, [rawData]);
  ...
  const productsForA = useMemo(() => ['All', ...Array.from(new Set(normalizedData.filter(d => d.brand_name === brandA).map(d => d.product_name))).sort()], [brandA, normalizedData]);
  const productsForB = useMemo(() => ['All', ...Array.from(new Set(normalizedData.filter(d => d.brand_name === brandB).map(d => d.product_name))).sort()], [brandB, normalizedData]);
  ...
  const computeMetrics = (targetBrand: string, targetProduct: string) => {
    const subset = normalizedData.filter(d => d.brand_name === targetBrand && (targetProduct === 'All' || d.product_name === targetProduct));
    ...
    Object.keys(stats).forEach(attr => averages[attr] = stats[attr].total / stats[attr].count);

    const tags = Object.keys(ingredientStats).map(ing => ({ name: ing, score: ingredientStats[ing].sentimentSum / ingredientStats[ing].count }));
  ...
  const anomalySubset = useMemo(() => normalizedData.filter(d => d.brand_name === brandA || d.brand_name === brandB), [normalizedData, brandA, brandB]);

  return (
    <main className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-8 w-full">
      <header>
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-white mb-2">Granular Product Comparison</h1>
        <p className="text-sm md:text-base text-slate-400">Head-to-head multi-axis benchmarking and formulation sentiment.</p>
      </header>

      <div className="bg-slate-900 border border-slate-800 p-2 md:p-6 rounded-xl w-full flex items-center justify-center">
        {/* Aspect Ratio box preserves scaling correctly on mobile */}
        <div className="w-full aspect-square max-h-[400px]">
          {metricsA || metricsB ? (
            ...
            <div className="flex h-full items-center justify-center text-slate-500 font-medium">Insufficient data for radar visualization.</div>
          )}
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        <div className="bg-slate-900 border border-slate-800 p-4 md:p-6 rounded-xl space-y-4">
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Parent Brand A</label>
            <select value={brandA} onChange={(e) => { setBrandA(e.target.value); setProductA('All'); }} className="w-full bg-slate-950 border border-slate-700 text-white rounded-lg px-4 py-3 md:py-2.5 focus:outline-none focus:border-sky-500">
          ...
          <div>
            ...
            <select value={productA} onChange={(e) => setProductA(e.target.value)} className="w-full bg-slate-950 border border-slate-700 text-white rounded-lg px-4 py-3 md:py-2.5 focus:outline-none focus:border-sky-500">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {metricsA.tags.map(tag => {
                      const colorClass = tag.score > 0.2 ? 'bg-emerald-950 text-emerald-400 border-emerald-800/50' : tag.score < -0.2 ? 'bg-rose-950 text-rose-400 border-rose-800/50' : 'bg-slate-800 text-slate-300 border-slate-700';
                      return <span key={tag.name} className={`px-2 py-1 rounded text-xs font-medium border ${colorClass}`}>{tag.name}</span>;
                    })}
                  </div>
                </div>
              )}
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 md:p-6 rounded-xl space-y-4">
          <div>
            ...
            <select value={brandB} onChange={(e) => { setBrandB(e.target.value); setProductB('All'); }} className="w-full bg-slate-950 border border-slate-700 text-white rounded-lg px-4 py-3 md:py-2.5 focus:outline-none focus:border-purple-500">
              {uniqueBrands.map(b => <option key={`B-brand-${b}`} value={b}>{b}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Target Focus Entity</label>
            <select value={productB} onChange={(e) => setProductB(e.target.value)} className="w-full bg-slate-950 border border-slate-700 text-white rounded-lg px-4 py-3 md:py-2.5 focus:outline-none focus:border-purple-500">
